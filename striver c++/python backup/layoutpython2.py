...
def add_new_row(frame, data=None):
    ...
    for col_index, col_name in enumerate(COLUMNS):
        if col_name == "S.No":
            lbl = tk.Label(frame, text=str(row_index), relief="ridge" , borderwidth=1 , width=15)
            ...
            cb = tk.Checkbutton(frame, variable=var, command=update_auto_labels , borderwidth=1,relief="groove",width=15)
            cb.grid(row=row_index, column=col_index, sticky="nsew")
            widgets.append(cb)

        elif "Notes" in col_name:
            txt = tk.Text(frame, height=2, width=20,relief="sunken", borderwidth=1)
            txt.grid(row=row_index, column=col_index, sticky="nsew")
            notes[col_name] = txt
            widgets.append(txt)

        elif "Last Performance" in col_name or "Next Review" in col_name:
            lbl = tk.Label(frame, text="(auto)", anchor='center', relief="sunken",borderwidth=1, width=15 )
            lbl.grid(row=row_index, column=col_index, sticky="nsew")
            labels[col_name] = lbl
            widgets.append(lbl)

        elif col_name == "Link":
            link_entry = tk.Entry(frame, width=15, relief="sunken", borderwidth=1  )
            link_entry.insert(0, DEFAULT_LINK)
            link_entry.grid(row=row_index, column=col_index, sticky="nsew")
            entries[col_name] = link_entry
            widgets.append(link_entry)

        else:
            entry = tk.Entry(frame, width=15, relief="sunken", borderwidth=1)
            entry.grid(row=row_index, column=col_index, sticky="nsew")
    ...
    delete_btn = tk.Button(frame, text="🗑️", command=delete_this_row , relief="raised",borderwidth=1)
    delete_btn.grid(row=row_index, column=len(COLUMNS), sticky="nsew")
    ...
    frame.grid_rowconfigure(row_index, pad=2)  # ✅ Clean row spacing
